...
export default function DashboardEDC() {
  const [listData, setListData] = useState<ListResponse<EDCData>>({ data: [], total: 0, page: 1, totalPages: 0 });
  ...
  const [filterOptions, setFilterOptions] = useState<{ units: string[], pms: string[], vendors: string[] }>({ units: [], pms: [], vendors: [] });

  const fetchStats = useCallback(async () => {
    try {
      const res = await DataService.getStats("edc");
      setStats(res);
    } catch (e) {
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await DataService.getList<EDCData>("edc", {
        page,
        limit: 5,
        search,
        unit,
      ...
      // Safety check: Pastikan res punya struktur yang benar
      setListData(res || { data: [], total: 0, page: 1, totalPages: 0 });
    } catch (e) {
      console.error("Gagal ambil list:", e);
    }
    setLoading(false);
  }, [page, search, unit, vendor, pemrakarsa, sort, order, filterType]);

  useEffect(() => {
    fetchStats();
    DataService.getFilterOptions("edc").then(res => {
  ...
  // Handle Loading state awal
  if (!stats && loading) return <div className="flex items-center justify-center h-64 font-bold text-[#00529C]">Memuat data...</div>;

  // Safety values agar tidak NaN
  const current = {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-xl font-extrabold text-[#1A202C] tracking-tight uppercase flex items-center gap-3">
            <div className="bg-[#00529C] w-1.5 h-6 rounded-full"></div>
            DASHBOARD MONITORING KANCA & UNIT 
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
        <StatCard label="TOTAL EDC" value={current.total} diff={getDiff(current.total, prev.total)} icon={<Users className="w-6 h-6 text-green-600" />} />
        <StatCard label="SALES VOL" value={`Rp ${current.sales_vol.toLocaleString('id-ID')}`} diff={getSVPDiff(current.sales_vol, prev.sales_vol)} icon={<TrendingUp className="w-6 h-6 text-red-600" />} />
        <StatCard label="TOTAL CASA" value={`Rp ${current.casa.toLocaleString('id-ID')}`} diff={getSVPDiff(current.casa, prev.casa)} icon={<Home className="w-6 h-6 text-pink-600" />} />
        <StatCard label="PRODUKTIF" value={current.productive} diff={getDiff(current.productive, prev.productive)} icon={<CheckCircle className="w-6 h-6 text-green-500" />} />
        <StatCard label="NON PRODUKTIF" value={current.total - current.productive} diff={getDiff(current.total - current.productive, prev.total - prev.productive)} icon={<XCircle className="w-6 h-6 text-red-500" />} />
      </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4">
            <h3 className="font-bold text-[#00529C] text-xs flex items-center gap-2 uppercase tracking-wide border-b border-gray-50 pb-3">
              <Search className="w-4 h-4" /> Filter Dashboard
            </h3>
            ...
            <div className="space-y-3">
              ...
              <FilterField label="Vendor EDC">
                ...
                >
                  ...
                  {filterOptions?.vendors?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </select>
              </FilterField>

              <FilterField label="Pemrakarsa">
                <select 
                  value={pemrakarsa}
              ...
              <FilterField label="Status Produktivitas">
                ...
                >
                  ...
                  <option value="produktif">Produktif (&gt;15jt)</option>
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between bg-white p-3 px-5 rounded-2xl shadow-sm border border-gray-100">
            <h3 className="font-bold text-[#1A202C] text-xs flex items-center gap-2">
              <Wallet className="w-4 h-4 text-[#00529C]" /> Daftar Performa Merchant
            </h3>
            <div className="relative w-48">
              <input 
                type="text" 
                placeholder="Cari TID / Nama..."
